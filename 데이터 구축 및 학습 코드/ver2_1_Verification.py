...
from ver2_1 import TCNClassifier, SignDataset, PreprocessedSignDataset, load_datasets, collate_fn

def analyze_ver2_1(model_path, base_dir='processed'):
    ...
    # 오답 기록용
    total_errors = 0
    num_errors = 0 # 실제 지숫자인데 틀린 경우
    char_errors = 0 # 실제 일반수어인데 틀린 경우

    print(f"\n=== ver2_1 전수 오답 분석 시작 (Soft Weighting 모델) ===")
    ...
    # 리포트 출력
    print("\n" + "="*60)
    print(f" [ver2_1 최종 분석 리포트]")
    print(f" 1. 전체 정확도: {correct/total:.2%} ({correct}/{total})")
    print(f" 2. 지숫자 판별 정답률 (Binary Type Acc): {binary_correct/total:.2%} ({binary_correct}/{total})")
    print("-" * 60)
...
if __name__ == "__main__":
    ...
    MODEL_FILE = 'best_ver2_1.pt'
    
    if os.path.exists(MODEL_FILE):
        try:
            analyze_ver2_1(MODEL_FILE, BASE_DIR)
        except Exception as e:
            print(f"\n[Error] 실행 중 오류가 발생했습니다: {e}")
            print(f"현재 BASE_DIR 설정: {BASE_DIR}")
            print("데이터 경로가 올바른지 확인해 주세요.")
    else:
        print(f"Error: {MODEL_FILE} 파일을 찾을 수 없습니다. 학습을 먼저 진행해 주세요.")