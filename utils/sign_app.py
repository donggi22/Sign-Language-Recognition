        ...
        try:
            ...
            with torch.no_grad():
                ...
                if confidence.item() >= self.confidence_threshold:
                    ...
                    return "불확실", confidence.item(), top3_pred
        except Exception as e:
...
def process_webcam(model, id_to_label, device, 
    ...
    try:
        ...
        while cap.isOpened() and not stop_requested:
            ...
            # 상태 표시
            if is_signing:
                status_text.markdown("🟢 **동작 감지 중**")
                # 빨간색 테두리 추가
                annotated_image = cv2.copyMakeBorder(
                    annotated_image, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=[255, 0, 0]
                )
            else:
                status_text.markdown("⚪ **대기 중**")
            
            # 결과 표시
            if result:
                result_text.markdown(f"## {result}")
                conf_text.markdown(f"신뢰도: {confidence:.2f}")
                
                # Top-3 예측 결과 표시
                for i, (pred, prob) in enumerate(top3_pred):
                    if i < 3:  # 최대 3개만 표시
                        top3_texts[i].markdown(f"{i+1}. **{pred}** ({prob:.2f})")
            
            # 예측 이력 표시
            if recognizer.prediction_history:
                history_md = "### 최근 예측:\n"
                for entry in reversed(recognizer.prediction_history[-5:]):  # 최근 5개만
                    history_md += f"- **{entry['label']}** ({entry['confidence']:.2f}) - {int((time.time() - entry['timestamp']))}초 전\n"
                history_text.markdown(history_md)
