...
def load_datasets(processed_dir='processed/', top_n_classes=994):
    def load_shards(split):
        ...
        for fname in sorted(os.listdir(shard_dir)):
            ...
            all_labels.extend([l.strip() for l in shard['labels']])  # 문자열 리스트
    ...
    # 상위 N 클래스 필터링 (공통 클래스 중에서 Train 빈도순)
    ...
    def filter_data(X, y_str_orig):
        # 1. label -> index 매핑 (정리 후 매핑, 없으면 -1)
        mapped = []
        for y in y_str_orig:
            cleaned = y.split('\n')[0].strip()
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = int(self.y[idx]) # tensor -> int로 변환
        return features.float(), label, {'data': [{'attributes': [{'name': self.idx_to_label[label]}]}]}
...
class PreprocessedSignDataset(Dataset):
    ...
    def rotation_augment(self, sequence, max_angle=15):
        ...
        # 4. rotation matrix (stack으로 생성 → 빠름)
        theta = torch.stack([
            torch.stack([cos, -sin]),
            torch.stack([sin,  cos])
        ]).squeeze()
...
class CrossModalAttention(nn.Module):
    def __init__(self, pose_dim, hand_dim, face_dim, embed_dim=128, num_heads=4):
        ...
        # 최종 융합 프로젝션
        # 입력: [pose(original), hand(original), face(original), attn_pose, attn_hand]
    ...
    def forward(self, pose_features, left_hand_features, right_hand_features, face_features):
        # inputs: [Batch, Dim, SeqLen] from Conv1d encoders
        
        # 1. 전처리: 손 특징 결합 및 차원 변경 (Permute for nn.MultiheadAttention: [SeqLen, Batch, Dim])
        pose = pose_features.permute(2, 0, 1) # [L, B, 128]
        hands = torch.cat([left_hand_features, right_hand_features], dim=1).permute(2, 0, 1) # [L, B, 256]
        face = face_features.permute(2, 0, 1) # [L, B, 256]
        ...
        # 3. Pathway A: Pose를 Query로 하여 Hands+Face 컨텍스트 참조
        context = torch.cat([h_emb, f_emb], dim=0) # [2L, B, 128]
        attn_out_pose, _ = self.attn_pose(p_emb, context, context)
        p_res = self.norm_pose(p_emb + self.dropout(attn_out_pose))
        
        # 4. Pathway B: Hand Self-Attention (손 모양 상세 강조)
        attn_out_hand, _ = self.attn_hand(h_emb, h_emb, h_emb)
        h_res = self.norm_hand(h_emb + self.dropout(attn_out_hand))
        
        # 5. 최종 융합 (Residual connection 형식으로 원본과 Attention 특징 결합)
        # 다시 Batch dimension을 앞으로 [B, L, Dim]
        ...
        fused = torch.cat([p_orig, h_orig, f_orig, p_res, h_res], dim=2) # [B, L, 128+256+256+128+128] = [B, L, 896]
        fused = self.out_proj(fused) # [B, L, 384]
        
        return fused.permute(0, 2, 1) # [B, 384, L]
...
class ModalityFusionModule(nn.Module):
    def __init__(self, pose_dim=75, left_hand_dim=63, right_hand_dim=63, face_dim=210):
        ...
        # CrossModalAttention 초기화 (pose_dim=128, hand_dim=256(left+right), face_dim=256)
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(processed_train_dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn, num_workers=NUM_WORKERS, pin_memory=True)
    ...
    class_counts = torch.bincount(y_train, minlength=len(label_map)).float()
