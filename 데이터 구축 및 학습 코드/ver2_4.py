...
def apply_3d_rotation(hand_tensor, yaw_range=20, pitch_range=20, roll_range=20):
    """
    Apply 3D Euler rotation to hand landmarks [T, 21, 3] relative to wrist (index 0).
    """
    T = hand_tensor.size(0)
    device = hand_tensor.device
    
    # 1. Random angles in radians
    yaw = random.uniform(-yaw_range, yaw_range) * (math.pi / 180.0)
    pitch = random.uniform(-pitch_range, pitch_range) * (math.pi / 180.0)
    roll = random.uniform(-roll_range, roll_range) * (math.pi / 180.0)
    
    # 2. Rotation Matrices
    def get_rot_matrix(y, p, r):
        cy, sy = math.cos(y), math.sin(y)
        cp, sp = math.cos(p), math.sin(p)
        cr, sr = math.cos(r), math.sin(r)
        
        # R = Rz(roll) * Ry(pitch) * Rx(yaw)
        # Note: Order matters. Here we use a standard Euler rotation.
        R = torch.tensor([
    ...
    # 3. Apply rotation (subtract wrist first)
    hand = hand_tensor.view(T, 21, 3)
    wrist = hand[:, 0:1, :]
    hand_shifted = hand - wrist
    
    # Batched matrix multiplication: [T, 21, 3] @ [3, 3]
    hand_rotated = torch.matmul(hand_shifted, R.t())
    
    return (hand_rotated + wrist).view(T, 63)
...
class EarlyStopping:
    def __init__(self, patience=10, verbose=False, delta=0, path='best_ver2_4.pt'):
...
class PreprocessedSignDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # For 3D Rotation, we need raw hand landmarks before SignDataset's normalization
        # but to keep it simple, we modify the SignDataset slightly or use orig data.
    ...
    def _apply_augmentation(self, f, aug_idx, is_num):
        ...
        if aug_idx == 2: # 3D Rotation Simulation (Virtual Angles)
            # Pose: [T, 75], LH_norm: [T, 63], LH_dist: [T, 5], RH_norm: [T, 63], RH_dist: [T, 5]
            # Hand start: pos 75 (LH), 143 (RH)
            ...
            res = f.clone()
            # Rotate LH (75:138) and RH (143:206)
            for start in [75, 143]:
        ...
        if aug_idx == 4: # Enhanced Noise (Simulating low light / shaking)
...
def train_epoch(model, loader, criterion, optimizer, device, scaler, scheduler=None):
    ...
    for s, l, n in tqdm(loader, desc='Training'):
        ...
        with torch.amp.autocast('cuda'):
            ...
            sample_weight = 1.0 + 2.0 * num_prob # Stronger weight for numeric in V2.4
            losses = criterion(out, l, per_sample=True)
            loss = (losses * sample_weight).mean()
            num_loss = bin_crit(num_logits, n)
            total_loss = loss + 0.5 * num_loss # Increased task weight
...
if __name__ == "__main__":
    ...
    s_weights = [2.0 if is_num else 1.0 for is_num in tr_ds_raw.is_numeric for _ in range(6)]
    ...
    criterion = ImprovedFocalLoss(alpha=cw.to('cuda'), gamma=2.0, smoothing=0.1) # Increased gamma for harder numeric samples
    
    optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=0.07) # Stronger WD
    epochs = 70
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=LR, steps_per_epoch=len(tr_loader), epochs=epochs)
    early_stop = EarlyStopping(patience=15, verbose=True, path='best_ver2_4.pt')
    ...
    if os.path.exists('best_ver2_4.pt'):
        model.load_state_dict(torch.load('best_ver2_4.pt'))
