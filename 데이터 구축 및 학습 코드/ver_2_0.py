...
class EarlyStopping:
    def __init__(self, patience=10, verbose=False, delta=0, path='best_ver2.0.pt'):
...
class SignDataset(Dataset):
    ...
    def _apply_digit_augmentation(self, f):
        ...
        if strategy == 1: # Speed Perturbation (Digits are sensitive to timing)
        ...
        if strategy == 2: # Hand Jitter (More noise for digits)
        ...
        if strategy == 3: # Frame Dropout
            ...
            if mask.any(): return f[mask].repeat(2, 1)[:150] # Simple dropout
        return f

    def _apply_standard_augmentation(self, f):
        strategy = random.randint(0, 3)
        if strategy == 1: return f + torch.randn_like(f)*0.02 # Subtle Noise
        if strategy == 2: m = f.clone(); m[:, ::2] = -m[:, ::2]; return m # Mirror
        return f

# --- 모델 구조 (Task-Separated Multi-Head) ---
...
class TaskSeparatedTCN(nn.Module):
    def __init__(self, num_classes):
        ...
        # 3. Binary Type Head (Digit vs Char)
        ...
        # 5. Digit Head (Reinforced with Temporal Attention)
    ...
    def forward(self, x):
        ...
        # 4. Separate Path
        # Char Path
        out_char = self.char_head(global_feat)
        
        # Digit Path (More complex)
        d_feat = self.digit_expert(shared)
        d_seq = d_feat.permute(2, 0, 1) # [T, B, 256]
...
def train_epoch(model, d_loader, c_loader, criterion, optimizer, device, scaler, num_steps):
    ...
    for _ in pbar:
        ...
        with torch.amp.autocast('cuda'):
            ...
            is_digit = (n > 0.5)
            # Multi-Head Loss with Label Smoothing
            loss = 0
        ...
        is_num_pred = torch.sigmoid(type_out) > 0.2 # Slight bias to digits
...
if __name__ == "__main__":
    ...
    d_loader = DataLoader(Subset(tr_ds, d_idx), batch_size=64, shuffle=True, collate_fn=collate_fn, num_workers=2, pin_memory=True)
    c_loader = DataLoader(Subset(tr_ds, c_idx), batch_size=64, shuffle=True, collate_fn=collate_fn, num_workers=2, pin_memory=True)
    ...
    early_stop = EarlyStopping(patience=12, verbose=True, path='best_ver2.0.pt')
    ...
    model.load_state_dict(torch.load('best_ver2.0.pt'))
