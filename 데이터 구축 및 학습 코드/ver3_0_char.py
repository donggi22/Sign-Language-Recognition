...
def load_datasets(processed_dir='processed/', top_n_classes=1500):
    ...
    # 지문자만 필터링 (is_numeric_label == False)
    char_labels = [lbl for lbl in y_tr_s if not is_numeric_label(lbl)]
    top_labels = {lbl: i for i, (lbl, _) in enumerate(Counter(char_labels).most_common()) if lbl in common}
...
class SignDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feat = torch.cat([p, lh_norm, lh_dist, lh_angle, rh_norm, rh_dist, rh_angle, f * self.face_weight], dim=1)
...
class PreprocessedSignDataset(Dataset):
    def __init__(self, dataset, split='train', augment=True):
        ...
        self.aug_factor = 5 if self.augment else 1
    ...
    def __getitem__(self, idx):
        ...
        if self.augment: feat = self._apply_augmentation(feat, aug_idx)
        return feat, label
...
class ModalityFusionModule(nn.Module):
    def __init__(self, pose_dim=75, hand_dim=150, face_dim=210):
        ...
        self.lh_enc = nn.Sequential(nn.Conv1d(75, 128, 3, padding=1), nn.BatchNorm1d(128), nn.ReLU())
...
if __name__ == "__main__":
    ...
    print(f"Num Char Classes: {len(label_map)}")
    
    tr_ds_raw = SignDataset(X_tr, y_tr, face_weight=2.0)
    tr_ds = PreprocessedSignDataset(tr_ds_raw, split='train', augment=True)
    
    tr_loader = DataLoader(tr_ds, batch_size=64, shuffle=True, collate_fn=collate_fn, num_workers=2, pin_memory=True)
    ...
    early_stop = EarlyStopping(patience=15, verbose=True, path='best_ver3_0_char.pt')
    scaler = torch.amp.GradScaler('cuda')

    print(f"Version 3.0 Character Classifier Training Started")
    ...
    if os.path.exists('best_ver3_0_char.pt'):
        model.load_state_dict(torch.load('best_ver3_0_char.pt'))
    _, ts_a, ts_5 = evaluate(model, ts_loader, criterion, 'cuda')
    print(f"\nFinal Test Acc (V3.0 Char): {ts_a:.2f}%, Top-5: {ts_5:.2f}%")
