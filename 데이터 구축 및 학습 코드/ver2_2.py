...
# Redefine FocalLoss for cleaner batch handling and label smoothing
class ImprovedFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, per_sample=False):
        ...
        # Cross Entropy with smoothing: sum(-y_true * log(y_pred))
        # Then multiply by focal weight for the specific target class
        # Note: focal loss usually applies the weight of the *target class* specifically
...
class EarlyStopping:
    def __init__(self, patience=10, verbose=False, delta=0, path='best_ver2_2.pt'):
...
class PreprocessedSignDataset(Dataset):
    ...
    def _apply_augmentation(self, f, aug_idx, is_num):
        ...
        if aug_idx == 2: # Noise + Global Scaling
            scale = random.uniform(0.9, 1.1)
            return (f * scale) + torch.randn_like(f) * 0.015
        ...
        if aug_idx == 4: # Global Rotation (Skeletal only rotation 모드)
            angle = random.uniform(-15, 15) * (math.pi / 180.0)
            cos_a, sin_a = math.cos(angle), math.sin(angle)
            R = torch.tensor([[cos_a, -sin_a], [sin_a, cos_a]], device=f.device)
            
            res = f.clone()
            for i in range(0, res.size(1) - 2, 3): # Process XYZ groups
                xy = res[:, i:i+2]
                res[:, i:i+2] = torch.matmul(xy, R.t())
            return res

        if aug_idx == 5: # Random Jitter (Gentle)
            return f + torch.randn_like(f) * 0.03
...
def train_epoch(model, loader, criterion, optimizer, device, scaler, scheduler=None):
    ...
    for s, l, n in tqdm(loader, desc='Training'):
        ...
        with torch.amp.autocast('cuda'):
            ...
            # Pass per_sample=True to get individual losses for weighting
...
if __name__ == "__main__":
    ...
    # Using ImprovedFocalLoss with Label Smoothing
    ...
    early_stop = EarlyStopping(patience=12, verbose=True, path='best_ver2_2.pt')
    ...
    if os.path.exists('best_ver2_2.pt'):
        model.load_state_dict(torch.load('best_ver2_2.pt'))
